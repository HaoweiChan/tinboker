...
| Backfilling old episodes, or generating episode content with a Claude session | `docs/workflows/episode-backfill.md` |
...
VPS `152.53.136.182` (Netcup, Debian 13), Caddy reverse proxy. The data lives there:
Postgres in Docker (`tinboker-postgres`, bound to `127.0.0.1:5433` — reach it with
`ssh -L 5433:127.0.0.1:5433 root@152.53.136.182`), and the media tree
`/srv/tinboker-media` that Caddy serves at `podcast-api.tinboker.com/media`. The
`graphfolio-articles` path segment in media URLs is a directory name left over from
GCS, not a bucket.

GCP project `gen-lang-client-0901363254` is **Secret Manager only**. Firestore, GCS and
Cloud SQL are gone — verified 2026-09-03, `gcloud firestore databases describe
--database=graphfolio-db` and `gcloud storage ls gs://graphfolio-articles` both 404, and
`gcloud sql instances list` is empty across every project on the account.
