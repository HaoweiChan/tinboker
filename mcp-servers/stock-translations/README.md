...
**Privileged (registered only when `TINBOKER_ADMIN_TOKEN` is set):**
...
| `TINBOKER_ADMIN_TOKEN` | _(unset)_ | Admin bearer JWT. When set, registers the privileged backfill tools. See [BACKFILL_AGENT.md](./BACKFILL_AGENT.md). |
