...
| `SYNDICATE_MAX_AGE_DAYS` (backend) | `7` | The syndicate endpoint refuses episodes released more than this many days ago unless the caller passes `allow_old=true`; unknown release date counts as old. Added after the 2026-09-04 backfill leak (817 old episodes published as new). `0` = off. The pipeline has a same-named gate of its own. |
