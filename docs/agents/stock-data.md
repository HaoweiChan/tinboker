...
- **Public return routes carry the same split guard.** `/batch-prices-since` nulls a ticker whose reference→latest span reaches a price break; `/batch-prices-trailing` nulls each d1/d7/d30/d90 window whose anchor predates the newest break and starts the sparkline `series` at that break. Both use one DB-only `_read_series_safe` read per ticker and fail closed (series read error → null). Same `PRICE_BREAK_BAND` limits as above. Both stay public — they feed edge/Redis-cached pages.
