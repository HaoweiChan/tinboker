...
- **BUG-9 (medium, historical):** [`backend/src/config.py`](../../backend/src/config.py) had `trendbrief.xyz` (old domain) but not `tinboker.com`. Spot-check before infra changes. See [`../issues.md`](../issues.md) BUG-9.
...
- Bugs: BUG-4, BUG-9, BUG-11, INFRA-1..4 in [`../issues.md`](../issues.md)
