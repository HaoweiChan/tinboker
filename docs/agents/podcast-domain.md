...
- **Two recommendation paths exist in parallel.** Legacy `/api/recommendations/*` (Postgres-backed, **soft-deprecated** — `deprecated=True`, kept as an alias for one release) and new `/api/ticker-insights/*` (Firestore-backed). New work goes through ticker-insights. See [`issues.md`](../issues.md) "Deprecated recommendation paths".
...
- Known bugs in this domain: [`../issues.md`](../issues.md) (BUG-10 recommendations endpoint URL, comment-type drift history)
