...
```bash
pytest tests/ -v --cov=src --cov-report=term-missing   # unit + integration, with coverage
pytest -m asyncio -v                                    # async-marked tests only
pytest -m integration -v                                 # integration-marked tests only
```
