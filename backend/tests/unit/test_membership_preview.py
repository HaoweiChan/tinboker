...
def test_preview_me_member_gate_refresh_and_restore(preview):
    client, user, token, write = preview
    actual = user.member_until
    deadline = None
    for mode in ("free", "paid", "original"):
        response = toggle(client, token, mode)
        assert response.status_code == 200
        data = response.json()
        token = data["token"]
        assert data["user"]["membership_preview_available"] is True
        assert data["user"]["membership_preview"] == (None if mode == "original" else mode)
        assert data["user"]["is_member"] is (mode != "free")
        assert user.member_until == actual and user.membership_preview is None
        payload = verify_jwt_token(token)
        deadline = deadline or payload["membership_preview_expires"]
        assert payload["exp"] == payload["membership_preview_expires"] == deadline
        ...
        assert refreshed.json()["user"] == data["user"]
        for name in ("token", "refresh_token"):
            assert verify_jwt_token(refreshed.json()[name])["exp"] == deadline
    assert data["user"]["member_until"] == user.model_dump(mode="json")["member_until"]
    write.assert_not_called()


@pytest.mark.parametrize("environment", ["staging", "production", "test"])
@pytest.mark.parametrize("mode", ["free", "paid", "original"])
...
def test_paid_preview_of_free_user_and_validation(preview):
    ...
    assert toggle(client, token, "original").json()["user"]["is_member"] is False
...
def test_expired_preview_rejected_even_with_future_token_exp(preview):
    client, _, token, _ = preview
    data = toggle(client, token, "paid").json()
    for name in ("token", "refresh_token"):
        payload = verify_jwt_token(data[name])
        payload["membership_preview_expires"] = int(datetime.now(timezone.utc).timestamp()) - 1
