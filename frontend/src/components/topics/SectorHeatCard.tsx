...
export const SectorHeatCard: React.FC<SectorHeatCardProps> = ({ exposureId, className }) => {
  ...
  return (
    <div className={cn('bg-card border border-border rounded-[10px] p-5 flex flex-col justify-between gap-4', className)}>
      <div className="flex items-center gap-2">
        <Flame size={14} className={hot ? 'text-accent-warning' : 'text-muted-foreground'} />
        <h3 className="text-xs text-muted-foreground">討論熱度與成分股表現</h3>
      </div>
      <div className="grid grid-cols-2 gap-4 items-end">
        <div>
          <div className="text-2xs text-muted-foreground mb-1">討論熱度排名</div>
        ...
        <div>
          <div className="text-2xs text-muted-foreground mb-1">成分股平均漲跌</div>
          <div className="text-2xl font-mono tabular-nums font-semibold">
        ...
        <div className="col-span-2">
          <div className="text-2xs text-muted-foreground mb-1">成分股近 12 日走勢</div>
          {series && series.length > 1 ? (
            <SimpleSparkline data={series} isPositive={(me.avg_change ?? 0) >= 0} color={trend.lineColor} width={260} height={56} smooth className="w-full" />
          ) : (
