...
export const SummaryMarkdown: React.FC<SummaryMarkdownProps> = ({ content, onSeek }) => {
  ...
  return (
    <div className="text-xl leading-[1.85] text-foreground/90">
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          h1: ({ children }) => (
            <h2 className="text-3xl sm:text-4xl font-semibold tracking-[-0.01em] leading-[1.35] mt-7 first:mt-0 mb-3">{children}</h2>
          ),
          ...
          h2: ({ children }) => (
            <h3 className="text-2xl sm:text-3xl font-semibold leading-[1.4] mt-6 mb-2.5">{children}</h3>
          ),
          h3: ({ children }) => (
            <h4 className="text-xl sm:text-2xl font-semibold text-foreground/95 mt-5 mb-2">{children}</h4>
          ),
          h4: ({ children }) => (
            <h5 className="text-lg sm:text-xl font-semibold text-foreground/90 mt-4 mb-1.5">{children}</h5>
          ),
