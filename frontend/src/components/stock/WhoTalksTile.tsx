...
interface WhoTalksTileProps {
  /** Count per podcaster from per-episode insights… */
  insights?: TickerInsight[];
  /** …or precomputed rows (e.g. episodes per show on a sector page). */
...
export const WhoTalksTile: React.FC<WhoTalksTileProps> = ({ insights, rows: given, title = '誰在談 · 90 天', unit, className, max = 4 }) => {
  const rows = useMemo(() => {
    const counts = new Map<string, number>();
    if (given) for (const r of given) counts.set(r.name, (counts.get(r.name) ?? 0) + r.n);
    else for (const i of insights ?? []) if (i.podcaster) counts.set(i.podcaster, (counts.get(i.podcaster) ?? 0) + 1);
      ...
      <div className="flex flex-col gap-2 text-sm">
        {rows.map((r, i) => (
          <Link key={r.name} to={`/podcaster/${encodeURIComponent(r.name)}`} className="group flex items-center gap-3 min-w-0">
            ...
            <span className="w-10 shrink-0 text-right text-xs font-mono tabular-nums text-muted-foreground">{r.n}{unit ?? ''}</span>
          </Link>
        ))}
      </div>
    </div>
