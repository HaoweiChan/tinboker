...
def test_non_us_ticker_uses_bare_doc_id():
    # Hermes-critical invariant: the external trading consumer reads
    # trending_tickers/{bare_ticker} directly — no market suffix, no
    # "market" field on the doc.
    docs = aggregate_trending(
        [_insight("2330", 0.8, 1, "股癌", "e1")],
        now=_NOW,
    )
