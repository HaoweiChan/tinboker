...
# is_relevant is the whole answer here — no prose is used downstream (only the boolean
# reaches ``relevance_map``), which is exactly the shape a decisions model answers for
# ~97% less than the same verdict generated by a chat model. Probability cutoff: the
# verifier exists to drop false positives, and Jev's probabilities are calibrated, so
# the neutral 0.5 is the honest threshold — raise it to filter harder.
_RELEVANCE_CUTOFF = 0.5


def _verify_relevance(exposures_json: list[dict[str, Any]]) -> dict[str, bool]:
    """Judge each candidate exposure, returning ``{sector_id: is_relevant}``.

    Two paths, chosen by how ``sector_verifier`` is configured: a decisions model
    (``decisions:typesafe/jev-1.13``) answers one Noul question per exposure, while a
    chat model keeps the original single batched JSON call. Both read their criteria
    from ``sector_verifier.yaml`` so the rules have exactly one owner. Raising is part
    of the contract — the caller's fallback decides what a verifier outage means.
    """
    from ..llm import decide, invoke_json, is_decisions_model, load_prompt

    prompts = load_prompt("sector_verifier")

    if is_decisions_model("sector_verifier"):
        question = {"is_relevant": {"type": "noul", "instructions": prompts["noul_instructions"]}}
        relevance: dict[str, bool] = {}
        for exp in exposures_json:
            answers = decide("sector_verifier", exp, question)
            relevance[exp.get("sector_id")] = float(answers["is_relevant"]["noul"]) >= _RELEVANCE_CUTOFF
        return relevance
    ...
    verifications = invoke_json("sector_verifier", messages).get("verifications", [])
    return {item.get("sector_id"): item.get("is_relevant", False) for item in verifications}
...
def derive_sector_exposures(state: PipelineState) -> dict[str, Any]:
    ...
    if to_verify:
        ...
        try:
            relevance_map = _verify_relevance(exposures_json)
