...
— there's no incremental state to maintain, and doc-ids are the bare ticker
token (``trending_tickers/2330``, no market suffix).
"""
...
# tickers.created_at COLLECTION_GROUP Firestore index (the eee31f0 delta path
# was dead because that index was never created). Doc-ids stay bare: the
# external trading consumer reads trending_tickers/{bare_ticker} directly.
